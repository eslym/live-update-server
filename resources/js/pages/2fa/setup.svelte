...
<script lang="ts">
    ...
    let { qr, secret, debug_code }: { qr: string; secret: string; debug_code: string | null } =
        $props();
            ...
            QRCode.toString(
                ...
                (err, svg) => {
                    if (err) {
                        ...
                        resolve(
                            svg
                                .replace('stroke="#000000"', 'class="stroke-purple-11"')
...
<div class="flex flex-col xl:flex-row gap-8 items-center max-w-4xl mx-auto w-full mt-8">
    ...
    <div class="flex-grow h-full">
        ...
        <p class="text-content2">
            You can also use the following code to set up your authenticator app:
        </p>
        <p class="mt-6 text-center">
            <input
                type="text"
                class="input input-xl input-solid font-mono text-center"
                value={secret}
                readonly
            />
        </p>
        ...
        <form
            class="mt-4 flex flex-row w-full gap-2"
            action="/2fa/setup"
            method="post"
            novalidate
            onsubmit={submit}

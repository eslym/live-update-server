...
<div class="w-full px-4 py-8 flex flex-col items-center justify-center">
    <form method="post" class="form-group max-w-96"
        ...
        <div class="form-field mt-8">
            <div class="form-control justify-between">
                {#if user['2fa_enabled']}
                    <label for="modal-disable-2fa" class="btn btn-solid-error w-full">Disable Two Factor
...
<div class="modal">
    ...
    <form action="/2fa/disable" method="post" class="modal-content flex flex-col gap-5 w-[24rem] max-w-full"
          ontransitionend={(ev) => {
              if(ev.currentTarget !== ev.target) return;
              if(disable2faModal) return;
              $disable2faForm.reset('password');
          }}
          onsubmit={disable2fa} novalidate>
        <label for="modal-disable-2fa" class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</label>
...
{#if recovery_codes}
    <input type="checkbox" class="modal-state" id="modal-recovery-codes" bind:checked={recoveryCodeModal}/>
    <div class="modal">
        <label class="modal-overlay" for="modal-recovery-codes"></label>
        <div class="modal-content flex flex-col gap-5 w-[28rem] max-w-full">
            <label for="modal-recovery-codes" class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</label>
            <h2 class="text-xl">Two Factor Auth Recovery Codes</h2>
            <p class="text-content2">
                Save these recovery codes in a secure place. They can be used to recover access to your account if you
                lose
                your two-factor authentication device. Each code can only be used once.
            </p>
